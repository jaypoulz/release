...
# Start Debugging - Delete me before merge
set -x
...
oc adm release extract -a "${CLUSTER_PROFILE_DIR}/pull-secret" \
  "${OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE}" --command=openshift-install --to=/tmp
...
echo "Sleeping 30m for debug purposes..."
sleep 30m
...
# Stop Debugging - Delete me before merge
set +x
